# Copyright (C) Lihao Tai and David Curtis
library(ggplot2)
library(reshape2)
library(zoo)
library(plyr)
...
plots_generator_p1 = function(plotting_data, method, png_file_name){
  
  results = data.frame(plotting_data)
  
  png_name = png_file_name
...
#define a function to generate the plots
plots_generator_p2 = function(plotting_data, method, png_file_name){
  
  results = data.frame(plotting_data)
  
  png_name2 = png_file_name
...
scripts_generator_pymol = function(ratio, given_table, script_file_name){
...
scripts_generator_pp = function(ratio, given_table, script_file_name){
  pred_str = vector()
  chi_value = vector()
  c_str = vector()
  df_filtered = given_table
...
if (filtered_table_file != "") {
  filterd_table_final = read.table(filtered_table_file, header = TRUE, stringsAsFactors = FALSE)
} else if (saofile != "") {
...
if (w_table != "") {
  write.table(filtered_table_final, w_table, sep = "\t", row.names = FALSE, quote = FALSE)
}

if (plot_1 != "") {
  plot_table = filtered_table_final
  plots_generator_p1(plot_table, Pred_Method,plot_1)
}

if (plot_2 != "") {
  plot_table = filtered_table_final
  plots_generator_p2(plot_table, Pred_Method,plot_2)
}
...
if (w_script_pymol != "") {
  if (saofile !="") {
    ...
    ratio=chi_ratio # could check here to see it is set
  }
  df_filtered = filtered_table_final
  scripts_generator_pymol(ratio, df_filtered, w_script_pymol)
}

if (w_script_pp != "") {
  df_filtered = filtered_table_final
  scripts_generator_pp(ratio, df_filtered, w_script_pp)
}
