...
if (length(args) < 1) {
  args = c(
    "--data_saofile", "/home/lihaotao/saofiles/UKBB.HT.forAnnot.20231112.SMAD6.txt", 
    "--filtered_table", "None",
    "--chi_ratio", "None",
    "--gene_name", "SMAD6",
    "--transcript", "NM_005585.5",
    "--input","saofile",
    "--output", "table+plots+scripts"  # New argument for output type
  )
}
...
npred = 0
a = 0
output_type = "table+plots+scripts"  # Default output type
...
while (TRUE) {
  ...
  if (arg == "--data_saofile") {
    saofile = args[a * 2 + 2]
  } else if (arg == "--filtered_table") {
    filtered_table = args[a * 2 + 2]
    ...
    output_type = args[a * 2 + 2]
  }
  a = a + 1
}

if (saofile == "None" & filtered_table != "None"){
  print("Your input file is a table.")
} else if (saofile != "None" & filtered_table == "None") {
  print("Your input file is a saofile.")
} else {
  print("There is something wrong with your input file. Please check. You could only give either a saofile or a table.")}

# Output based on user selection
if (output_type == "table") {
  print("Generating filtered table...")
  # Code to generate and output filtered table goes here
  
} else if (output_type == "plots+scripts") {
  print("Generating plots and scripts...")
  # Code to generate and output plots goes here
  
} else if (output_type == "table+plots+scripts") {
  print("Generating filtered table, plots, and scripts...")
  # Code to generate filtered table and plots goes here
}
...
if (saofile != "None" & grepl("table", output_type)) {
...
# save the table
table_name = sprintf("/home/lihaotao/saofiles/filtered_table_%s.txt",gene_name)
...
plots_generator = function(plotting_data){
  results = data.frame(read.table(plot_table, header = TRUE, stringsAsFactors = FALSE))
  
  png_name = sprintf("new_aa_plot_%s.png", gene_name)
  ...
  png_name2 = sprintf("new_aa_cc_plot_%s.png", gene_name)
...
scripts_generator <- function(ratio, df_filtered){
  ...
  aa_num_final_name = sprintf("/home/lihaotao/saofiles/new_aa_num_str_f_%s.txt", gene_name)
  pred_str_name = sprintf("/home/lihaotao/saofiles/new_pred_str_%s.txt", gene_name)
...
if (saofile != "None" & grepl("plots", output_type)) {
  plot_table = table_name
  plots_generator(plot_table)
}else if (filtered_table != "None" & grepl("plots", output_type)){
  plot_table = filtered_table
  plots_generator(plot_table)
}

if (saofile != "None" & grepl("scripts", output_type)) {
...
}else if (filtered_table != "None" & grepl("scripts", output_type)){
  ratio = chi_ratio
